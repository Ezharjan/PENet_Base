...
def depth_read(filename, args, modal_missing, epoch):
...
class KittiDepth(data.Dataset):
    ...
    def __getraw__(self, index):
        ...
        if(modal_missing == True):
            sparse = depth_read(self.paths['d'][index].replace('/KITTI/','/KITTI5/'), self.args, modal_missing, self.epoch) if (self.paths['d'][index] is not None and self.args.use_d) else None
        else:
            sparse = depth_read(self.paths['d'][index], self.args, modal_missing, self.epoch) if (self.paths['d'][index] is not None and self.args.use_d) else None
